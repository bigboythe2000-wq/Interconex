...
PESTANA_ENTRADA = "Debito Inmediato"       

ARCHIVO_EDITABLE = obtener_ruta_absoluta("Resultados.xlsx")
PESTANA_DESTINO = "Debito Inmediato" 
# ----------------------------------------
...
def obtener_otp_swagger(p, registros):
    ...
    try:
        ...
        for i, fila in enumerate(registros):
            id_pagador = str(fila.get('id_cliente_otp', '')).strip()
            id_numero = str(fila.get('id_numero_otp', '')).strip()
            telf_pagador = str(fila.get('telefono_otp', '')).strip().replace('´', '')
            ...
            print(f"🔍 [{i+1}] Solicitando OTP para Pagador: {id_pagador} | Teléfono: {telf_pagador} | Cedula: {id_numero}")
            ...
            payload_otp = {
                ...
                "idTerminal": "123BE",
                "idConsumidor": "J1234",
                "idCliente": id_pagador,
                "idUsuario": "J876543210",
                "codUsuario": "E7654",
                "ip": "172.1.1.1",
                "datosOTP": {
                    "nroIdCliente": id_numero,
...
# ======================================
#  FASE 2: EJECUTAR EL DEBITO INMEDIATO 
# =======================================
def ejecutar_debito_inmediato(p, registros):
    print("\n🚀 [FASE 2] Iniciando procesamiento de Debito inmediato...")
    browser = p.chromium.launch(headless=True)
    context = browser.new_context(viewport={'width': 1280, 'height': 800})
    page = context.new_page()
    ...
    try:
        ...
        columnas_texto = [
            "env_id_Cliente_OTP", "env_id_numero_OTP", "env_telefono_OTP", 
        ]
        ...
        for i, fila in enumerate(registros):
            ...
            if not id_cliente:
                print(f"❌ ERROR: La fila {i+1} no tiene valor en 'id_cliente_otp'")
                continue
                
            otp_usar = str(fila.get('otp_encontrada', '')).strip()
            estado_previo = str(fila.get('estado_otp', '')).strip()
            
            print(f"🔄 [{i+1}/{len(registros)}] Procesando Debito inmediato: {id_cliente}")
            marca_tiempo = dateAndHourNow()
            
            if not otp_usar or "Error" in estado_previo or "Sin" in estado_previo:
                print(f"⏭️ Saltando Debito inmediato debido a falta de OTP válida.")
                fila_res = {
            ...
            # JSON estructurado para el debito inmediato
            id_canal = str(fila.get('id_canal', '1')).strip()
            datos_peticion = {
                "idCliente": id_cliente,
                "idSesion": "20220430103200",
                "idCanal": int(id_canal),
                "idUsuario": "",
                "idTerminal": "",
                "ipCliente": "0.0.0.0",
                "idConsumidor": "",
                "ipConsumidor": "0.0.0.0"}
            # Solo asignar valor cuando el canal sea 2
            if id_canal == "2":
                datos_peticion["idConsumidor"] = str(fila.get("id_consumidor", "")).strip()
            payload_debito = {"datosPeticion": datos_peticion,
                              "debitoInmediato": {
                                  "otp": otp_usar,
            ...
            datos_enviados_planos = aplanar_dict(payload_debito, parent_key='Env')
        ...
        browser.close()
        #print(f"Cantidad de registros recibidos: {len(registros)}")
        #print(f"Cantidad de resultados generados: {len(resultados_finales)}")
...
def main():
    ...
    with sync_playwright() as p:
        ...
        registros_finales = ejecutar_debito_inmediato(p, datos_con_otp)
    ...
    print("🏁 Proceso de Debito inmediato Finalizado.")
